...
struct sTextureAsset {
	...
	const eTextureBasicFilter TextFilter{eTextureBasicFilter::NONE};
	const bool NeedAnisotropy{false};
	const bool MipMap{false};
	// preload
	GLtexture PreloadedTexture{0};

	sTextureAsset(bool _Alpha, eAlphaCreateMode _AlphaMode,
		      eTextureWrapMode _TextWrap, eTextureBasicFilter _TextFilter,
		      bool _NeedAnisotropy, bool _MipMap) :
		Alpha{_Alpha},
...
void ForEachTextureAssetLoad(std::function<void (unsigned AssetValue)> function)
{
	...
	for (auto &tmpAsset : TextureMap) {
		vw_SetTextureProp(tmpAsset.second.TextFilter,
				  tmpAsset.second.NeedAnisotropy ? GameConfig().AnisotropyLevel : 1,
				  tmpAsset.second.TextWrap, tmpAsset.second.Alpha,
