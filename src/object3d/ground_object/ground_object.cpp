...
void cGroundObject::SetRotation(const sVECTOR3D &NewRotation)
{
    ...
    if (!WeaponSlots.empty()) {
        for (auto &tmpWeaponSlot : WeaponSlots) {
            ...
            if (auto sharedWeapon = tmpWeaponSlot.Weapon.lock()) {
                if (TargetHorizChunkNums.empty() && TargetVertChunkNums.empty() && !DoNotCalculateRotation) {
...
bool cGroundObject::Update(float Time)
{
    ...
    if (WeaponTargeting) {
        ...
        if (FindTargetLocationWithPrediction(ObjectStatus, Location + FirePos, WeapNum, tmpTargetLocation)
            && GetTurretOnTargetOrientation(Location + FirePos, Rotation, CurrentRotationMat, tmpTargetLocation, NeedAngle)) {
    ...
    if (!TargetVertChunkNums.empty() && TargetVertChunksNeedAngle != TargetVertChunksCurrentAngle) {
        ...
        for (auto tmpChunkNum : TargetVertChunkNums) {
            ...
            sVECTOR3D tmpLocation = Chunks[tmpChunkNum].Location - Chunks[TargetVertChunkNums[0]].Location;
    ...
    if (!WeaponSlots.empty()) {
        ...
        if (WeaponFireType == 1) {
            ...
            for (unsigned i = 0; i < WeaponSlots.size(); i++) {
                if (!WeaponSlots[i].Weapon.expired() && WeaponSlots[i].SetFire) {
                    if (WeaponGroupCurrentFireNum == static_cast<int>(i) && WeaponGroupCurrentFireDelay <= 0.0f) {
                        if (auto sharedWeapon = WeaponSlots[i].Weapon.lock()) {
                            sharedWeapon->WeaponFire(Time);
                        }
    ...
    if (!WheelChunkNums.empty() && (fabs(WheelRotarySpeed) >= Epsilon)) {
    ...
    if (TrackChunkNum != -1 && fabs(WheelRotarySpeed) >= Epsilon) {
