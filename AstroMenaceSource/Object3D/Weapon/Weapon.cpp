...
void CWeapon::Create(int WeaponNum)
{
	...
	if (WeaponNum >= 1 && WeaponNum <= 99)
	{
		...
		LoadObjectData(PresetEarthWeaponData[WeaponNum-1].NameVW3D, this, PresetEarthWeaponData[WeaponNum-1].ObjectNum+1);
			...
			case 201:
				{
					...
					LoadObjectData(PresetPirateWeaponData[IntWeaponNum-1].NameVW3D, this, 0);
					for (int i=0; i<DrawObjectQuantity; i++)
					{
...
bool CWeapon::Update(float Time)
{
	...
	if (DestroyedFire == 0)
	{
		...
		DestroyedFire->Texture = vw_FindTextureByName("DATA/GFX/flare1.tga");
	...
	if (DestroyedSmoke == 0)
	{
		...
		DestroyedSmoke->Texture = vw_FindTextureByName("DATA/GFX/flare1.tga");
	...
	if (Ammo > 0 || GameLimitedAmmo != 1)
	{
		...
		{
			float fVol = 1.0f;
			Audio_PlaySound(SoundNum, fVol, 1.0f, Projectile->Location, false);
		}
	...
	if (WeaponTurret)
	{
		...
		//if (Fire != 0)
		{
			...
			GetTurretOnTargetOrientateion(ObjectStatus, Location+FireLocation, Rotation,
					CurrentRotationMat,	&NeedAngle, ObjectCreationType, Location);
...
bool CWeapon::WeaponFire(float Time)
{
	...
	if (ObjectCreationType >= 1 && ObjectCreationType <= 99)
	{
		...
		if (Strength <= 0.0f || Ammo == 0 || Misfire)
		{
			...
			switch (ObjectCreationType)
			{
				...
				case 3:
				case 4:
					Audio_PlaySound(1, fVol, 1.0f, Location, false);
					break;
				...
				case 9:
				case 10:
					Audio_PlaySound(2, fVol, 1.0f, Location, false);
					break;
				...
				// Laser
				case 14:
					Audio_PlaySound(3, fVol, 1.0f, Location, false);
				...
				// Gauss
				case 15:
					Audio_PlaySound(4, fVol, 1.0f, Location, false);
					break;
				...
				// бомба
				case 19:
					Audio_PlaySound(5, fVol, 1.0f, Location, false);
					break;
			}
	...
	if (SoundNum != 0)
	{
		...
		LaserMaserSoundNum = Audio_PlaySound(SoundNum, fVol, 1.0f, Location+FireLocation, false);
